...
if __name__ == "__main__":
    ...
    hfp_adas = create_adaboosts(hfp_x_train, hfp_y_train, n_estimators_set, .1)
    ...
    print('\nHeart Failure Prediction - Benchmarks (Boosting):')
    benchmarks(hfp_x_train, hfp_y_train, hfp_x_test, hfp_y_test, n_estimators_set, hfp_test_mse, create_adaboost)
    ...
    plot.add_plot(n_estimators_set, hfp_train_mse, 'training data (lr=0.1)', 'None')
    plot.add_plot(n_estimators_set, hfp_test_mse, 'testing data (lr=0.1)', 'None')
    plot.save()

    # Create Heart Failure Prediction NNs
    rwq_adas = create_adaboosts(rwq_x_train, rwq_y_train, n_estimators_set, .1)

    # Get training/testing accuracy for Heart Failure Prediction
    rwq_train_mse = get_mse(rwq_adas, rwq_x_train, rwq_y_train)
    rwq_test_mse = get_mse(rwq_adas, rwq_x_test, rwq_y_test)

    print('\nRed Wine Quality - Benchmarks (Boosting):')
    benchmarks(rwq_x_train, rwq_y_train, rwq_x_test, rwq_y_test, n_estimators_set, rwq_test_mse, create_adaboost)
    ...
    plot.add_plot(n_estimators_set, rwq_train_mse, 'training data (lr=0.1)', 'None')
    plot.add_plot(n_estimators_set, rwq_test_mse, 'testing data (lr=0.1)', 'None')
    plot.save()